...
- **状態**: 対応済み（完了済み）
- **実装**:
  - `backend/src/api/middleware/security.ts` で Redis 共有カウンタを使い、認証系と変更系 API のレート制限を適用。
...
- **状態**: 対応済み（完了済み）
- **実装**:
  - `backend/src/redis/authStore.ts` と `backend/src/features/auth/sessionStore.ts` で Redis 永続化へ移行。
  - セッション / OAuth State は TTL 付き Redis キーで期限管理し、複数インスタンス間でも共有できる。
